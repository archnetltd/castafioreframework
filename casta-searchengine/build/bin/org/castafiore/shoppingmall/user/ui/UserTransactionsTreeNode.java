...
public class UserTransactionsTreeNode implements MallTreeNode{
	/** The Constant ACTIONS. */
	private final static String[] ACTIONS = new String[]{"All Orders","In Cart", "Waiting Confirmation", "On Delivery", "Delivered",  "Cancelled by You", "Cancelled by merchant", "Conflict"};
	private final static String[] STATES = new String[]{"-1","9", "11", "12", "15",  "14", "13", "16"};
	...
	public TreeNode<Container> getNodeAt(final int index) {
		...
		return new MallTreeNode() {
			...
			@Override
			public Container getComponent() {
				return new EXContainer(action, "a").setText(action).setAttribute("href", "#_io").setAttribute("status", STATES[index]);
			}
	...
	@Override
	public void executeAction(Container source) {
		source.getAncestorOfType(EXMall.class).showOrdersHistory(source.getAttribute("status"));
