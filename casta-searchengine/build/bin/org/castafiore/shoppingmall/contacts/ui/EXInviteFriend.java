...
public class EXInviteFriend extends EXXHTMLFragment implements EventDispatcher{
	...
	public EXInviteFriend(String name)throws Exception {
		...
		.addChild(new EXInput("to","").addClass("span-12"))
		.addChild(new EXInput("subject").addClass("span-12"))
		.addChild(new EXTextArea("message").setAttribute("rows", "16").addClass("span-12"))
		.addChild(new EXContainer("send", "button").setText("Send Invitation").addEvent(DISPATCHER, Event.CLICK))
	...
	public void sendMessage(){
		ShoppingMallUser user = MallUtil.getCurrentUser();
		Message message= user.createMessage("created on " + new Date().toString());
