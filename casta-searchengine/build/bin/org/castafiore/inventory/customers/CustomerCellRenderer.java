...
public class CustomerCellRenderer implements CellRenderer, Event{

	@Override
	public Container getComponentAt(int row, int column, int page,
			TableModel model, EXTable table) {
		Customer c = (Customer)model.getValueAt(column, row, page);
	...
	public void onChangePage(Container result, int row, int column,
		...
		if(column == 0){
			...
			result.setAttribute("username", c.getUsername());
			result.setText(c.getFirstName() + " " + c.getLastName());
		}else if(column == 2){
			result.setText("0");
		}
	...
	public boolean ServerAction(Container container, Map<String, String> request)
			throws UIException {
		try{
			BCustomerForm form = new BCustomerForm("editCustomer");
			form.setCustomer((Customer)SpringUtil.getSecurityService().loadUserByUsername(container.getAttribute("username")));
