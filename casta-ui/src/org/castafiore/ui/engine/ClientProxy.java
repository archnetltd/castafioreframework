...
public final class ClientProxy implements Constant {
	...
	//private List<ClientProxy> children_ = new ArrayList<ClientProxy>(5);
	
	//private ClientProxy parent_;
	...
	public ClientProxy setTimeout(ClientProxy function, int milliseconds){
		this.appendJSFragment("setTimeout('" + JavascriptUtil.javaScriptEscape(function.getCompleteJQuery()) + "', "+ milliseconds +")");
		return this;
	}
	public ClientProxy setTimeout(Function function, int milliseconds){
		this.appendJSFragment("setTimeout('" + JavascriptUtil.javaScriptEscape(function.getJavascript()) + "', "+ milliseconds +")");
		return this;
	}
	/**
	...
	public ClientProxy getScript(String scriptUrl, ClientProxy callBack){
		return executeFunction(Constant.NO_CONFLICT + ".getScript",scriptUrl, new Function(callBack,new Var("data"), new Var("textStatus"), new Var("jqxhr")));
	}
	
	public ClientProxy getScript(JArray sciprts, ClientProxy callBack){
		return executeFunction(Constant.NO_CONFLICT + ".getScript",sciprts, new Function(callBack,new Var("data"), new Var("textStatus"), new Var("jqxhr")));
	}
	...
	public List<ClientProxy> getChildren()
	{
		...
		List<ClientProxy> result = new ArrayList<ClientProxy>(containers.size());
	...
	private static KeyValuePair makeKeyValuePair(String methodname, ClientProxy jquery) {
		SimpleKeyValuePair kv = new SimpleKeyValuePair();
		kv.setKey(methodname);
	...
	public ClientProxy setSVGAttributes(JMap map, String id){
		...
		while(keys.hasNext()){
			...
			if(!key.equalsIgnoreCase("name") && !key.equalsIgnoreCase("__path")){
				...
				appendJSFragment("_"+id+".setAttributeNS(null,\""+key+"\", "+val+");");
			}
		}
		return this;
	...
	public ClientProxy animate(JMap options, String duration, String easing, ClientProxy callback)
	{
		KeyValuePair kv = null;
		if(easing != null)
	...
	public static String getCurrentJQuery_(List<KeyValuePair> commands, String idRef)
	{
		...
		if(commands.size() > 0)
		{	
			result = result + Constant.NO_CONFLICT + "(" + ClientProxy.addQuote(idRef) + ")";
...
//		}
	...
	public ClientProxy makeServerRequest( JMap optionalParams, Event evt, String confirm)
	{
	...
	@Deprecated
	public ClientProxy makeServerRequest( String ancestorId,  JMap optionalParams, Event evt)
	{
	...
	public ClientProxy makeServerRequest( String ancestorId,  JMap optionalParams, Event evt, String confirm){
