 ...
 * Oct 22, 2008
 */
public final class CastafioreEngineGoogleApp {
	
	public static final Log logger = LogFactory.getLog(CastafioreEngineGoogleApp.class);
	 
	public static synchronized void executeInterceptorOnRender(Interceptor interceptor, Container container)
	{
	...
	public static synchronized String getJQuery(Container container, String parentId, Container root,  ListOrderedMap buffer)
	{
	...
	public static Event getEvent(String eventId, Container ancestor){
		
		String[] parts = StringUtil.split(eventId, "|");
		final String path = parts[0];
		final String type = StringUtil.split(parts[1], "[")[0];
		final String index = StringUtil.split(parts[1], "[")[1].replace("]", "");
		final List<Event> result = new ArrayList<Event>(1);
		ComponentUtil.iterateOverDescendentsOfType(ancestor, Container.class, new ComponentVisitor() {
			
			@Override
			public void doVisit(Container c) {
				if(c.getAttribute("__path").equals(path)){
					Event event = c.getEvents().get(Integer.parseInt(type)).get(Integer.parseInt(index));
					result.add(event);
				}
	 ...
	 */
	public static synchronized String executeServerAction(String componentId,  Application application, String parentId, Map<String, String> parameters)
	{
		...
		//eventid is the path__|type[index]
...
//				 }
	 ...
	 * @return
	 */
	private static synchronized Container[] getPrevAndNextSibblings(Container container)
	{
	 ...
	 * @param jQuery	
	 */
	public static synchronized void executeClientPartOfEvents(Container container, Container root, ClientProxy jQuery)
	{
	...
	private static synchronized JMap getMap(Container tag, int type)
	{
		...
		if(type == 0)
		{
			if(tag.rendered())
			{
				...
				{
					result.put(key, tag.getStyle(key));
				}
			}
			else
			{
