...
function getBrowserWidth(){var myWidth=0;
...
$.fn.smallHtmlBox=function(options){$(this).htmlbox().button("cut").button("copy").button("paste").button("bold").button("italic").button("underline").button("undo").button("redo").button("left").button("center").button("right").button("justify").button("fontsize",2).button("fontfamily",2).button("fontcolor",2).button("hyperlink").button("ol").button("ul").init()
};
$.fn.highlightForm=function(options){$(".contentA .left, .contentA input, .contentA textarea, .contentA select").focus(function(){$(this).parents(".row").addClass("ui-state-focus")
}).blur(function(){$(this).parents(".row").removeClass("ui-state-focus")
})
};
jQuery(document).bind("keydown","space",function(evt){jQuery(".ctrl_sensitive").attr("ctrlbtn","true")
});
jQuery(document).bind("keyup","space",function(evt){jQuery(".ctrl_sensitive").attr("ctrlbtn","false")
});
if(jQuery){(function(){$.extend($.fn,{rightClick:function(handler){$(this).each(function(){$(this).mousedown(function(e){var evt=e;
$(this).mouseup(function(){$(this).unbind("mouseup");
if(evt.button==2){handler(evt);
return false
}else{return true
}})
});
...
return $(this)
},rightMouseDown:function(handler){$(this).each(function(){$(this).mousedown(function(e){if(e.button==2){handler($(this));
return false
}else{return true
}});
$(this)[0].oncontextmenu=function(){return false
...
return $(this)
},rightMouseUp:function(handler){$(this).each(function(){$(this).mouseup(function(e){if(e.button==2){handler($(this));
return false
}else{return true
}});
$(this)[0].oncontextmenu=function(){return false
...
var funcSelect=options.onSelect;
funcSelect(item)
}
...
$.fn.htmlbox=function(){if(undefined===window.glob_ha){glob_ha=[]
...
var d={buttons:[],idir:"htmlBox/images/",images:[],output:"xhtml",css:"body{margin:3px;font-family:verdana;font-size:11px;}p{margin:0px;}",rows:[],success:function(data){alert(data)
},error:function(a,b,c){return this
}};
if(!$(this).attr("id")){$(this).attr("id","jqha_"+glob_ha.length);
d.id="jqha_"+glob_ha.length;
glob_ha[glob_ha]=glob_ha
}else{d.id=$(this).attr("id")
}if(undefined===glob_ha[d.id]){glob_ha[d.id]=this
}var get_selection=function(){var range;
if($.browser.msie){range=d.iframe.contentWindow.document.selection.createRange();
if(range.htmlText&&range.text){return range.htmlText
}}else{if(d.iframe.contentWindow.getSelection){var selection=d.iframe.contentWindow.getSelection();
if(selection.rangeCount>0&&window.XMLSerializer){range=selection.getRangeAt(0);
...
var in_array=function(o,a){for(var i in a){if((i===o)){return true
}}return false
};
var insert_text=function(text,start,end){if($.browser.msie){if(typeof d.idoc.selection!=="undefined"&&d.idoc.selection.type!=="Text"&&d.idoc.selection.type!=="None"){start=false;
d.idoc.selection.clear()
}var sel=d.idoc.selection.createRange();
sel.pasteHTML(text);
if(text.indexOf("\n")===-1){if(start===false){}else{if(typeof start!=="undefined"){sel.moveStart("character",-text.length+start);
sel.moveEnd("character",-end)
}else{sel.moveStart("character",-text.length)
}}sel.select()
}}else{d.idoc.execCommand("insertHTML",false,text)
}};
var keyup=function(e){var html=$("#1"+d.id).is(":visible")?$("#"+d.id).val():html=d.iframe.contentWindow.document.body.innerHTML;
html=(typeof getXHTML==="function")?getXHTML(html):html;
$("#"+d.id).val(html);
if(undefined!==d.change){d.change()
}};
var keypress=function(e){};
var style=function(){if(d.idoc.createStyleSheet){d.idoc.createStyleSheet().cssText=d.css
}else{var css=d.idoc.createElement("link");
css.rel="stylesheet";
css.href="data:text/css,"+escape(d.css);
if($.browser.opera){d.idoc.documentElement.appendChild(css)
}else{d.idoc.getElementsByTagName("head")[0].appendChild(css)
}}};
var wrap_tags=function(start,end){var sel=get_selection();
if(undefined===sel){sel=""
}insert_text(start+sel+end,start.length,end.length)
};
this.init=function(is_init){if(undefined===is_init){setTimeout("glob_ha['"+d.id+"'].init(true)",250);
return false
...
$("."+d.id+"_tb").find("button").each(function(){$(this).css("border","1px solid #E9EAEF").css("background","transparent").css("margin","1px 1px 1px 1px").css("padding","1px");
...
$("."+d.id+"_tb").find("select").each(function(){$(this).css("border","1px solid #E9EAEF").css("background","transparent").css("margin","2px 2px 3px 2px");
...
if($.browser.mozilla||$.browser.safari){d.idoc.open("text/html","replace");
d.idoc.write(text);
d.idoc.close()
}else{if(text!==""){d.idoc.write(text)
...
}if(d.idoc.createStyleSheet){setTimeout("glob_ha['"+d.id+"'].set_text(glob_ha['"+d.id+"'].get_html())",10)
}else{var css=d.idoc.createElement("link");
css.rel="stylesheet";
css.href="data:text/css,"+escape(d.css);
if($.browser.opera){d.idoc.documentElement.appendChild(css)
}else{if($.browser.safari){}else{d.idoc.getElementsByTagName("head")[0].appendChild(css)
}}}if(iframe.contentWindow.document.attachEvent){iframe.contentWindow.document.attachEvent("onkeyup",keyup);
iframe.contentWindow.document.attachEvent("onkeypress",keypress)
}else{iframe.contentWindow.document.addEventListener("keyup",keyup,false);
iframe.contentWindow.document.addEventListener("keypress",keypress,false)
}$(this).hide()
}catch(e){alert("This rich text component is not supported by your browser.\n"+e);
$(this).show()
}return this
};
this.change=function(fn){d.change=fn;
return this
};
this.remove=function(){glob_ha[d.id]=undefined;
$(d.id+"_wrap").remove()
};
var toolbar=function(){var h="";
for(var k=1;
k<d.rows.length;
k++){if(undefined===d.rows[k]){continue
}var buttons=d.rows[k].split(",");
h+="<tr><td class='"+d.id+"_tb' valign='middle' style='background:silver;border-bottom:1px outset white'>";
for(var i=0;
i<(buttons.length+1);
i++){if(undefined===d.buttons[buttons[i]]){continue
...
this.cmd=function(cmd,arg1){d.iframe.contentWindow.focus();
if(cmd==="paragraph"){cmd="format";
arg1="p"
...
}if(cmd==="code"){var text=this.get_html();
if($("#"+d.id).is(":visible")){$("#"+d.id).hide();
$("#"+d.id+"_html").show();
this.set_text(text)
}else{$("#"+d.id).show();
$("#"+d.id+"_html").hide();
this.set_text(text);
$("#"+d.id).focus()
}}else{if(cmd==="hyperlink"){d.idoc.execCommand("createlink",false,prompt("Paste Web Address URL Here:"))
}else{if(cmd==="image"){d.idoc.execCommand("insertimage",false,prompt("Paste Image URL Here:"))
}else{if(cmd==="fontsize"){d.idoc.execCommand(cmd,false,arg1)
}else{if(cmd==="backcolor"){if($.browser.msie){d.idoc.execCommand("backcolor",false,arg1)
}else{d.idoc.execCommand("hilitecolor",false,arg1)
}}else{if(cmd==="fontcolor"){d.idoc.execCommand("forecolor",false,arg1)
}else{if(cmd==="fontname"){d.idoc.execCommand(cmd,false,arg1)
}else{if(cmd==="cut"){if($.browser.msie===false){alert("Available in IExplore only.\nUse CTRL+X to cut text!")
}else{d.idoc.execCommand("Cut")
}}else{if(cmd==="copy"){if($.browser.msie===false){alert("Available in IExplore only.\nUse CTRL+C to copy text!")
}else{d.idoc.execCommand("Copy")
}}else{if(cmd==="paste"){if($.browser.msie===false){alert("Available in IExplore only.\nUse CTRL+V to paste text!")
}else{d.idoc.execCommand("Paste")
}}else{if(cmd==="format"){if(arg1==="pindent"){wrap_tags('<p style="text-indent:20px;">',"</p>")
}else{if(arg1!==""){d.idoc.execCommand("formatBlock",false,"<"+arg1+">")
}}}else{if(cmd==="striptags"){var sel=get_selection();
sel=sel.replace(/(<([^>]+)>)/ig,"");
insert_text(sel)
}else{if(cmd==="quote"){wrap_tags('<br /><div style="position:relative;top:10px;left:11px;font-size:11px;font-family:verdana;">Quote</div><div class="quote" contenteditable="true" style="border:1px inset silver;margin:10px;padding:5px;background:#EFF7FF;">',"</div><br />")
}else{if(cmd==="styles"){wrap_tags(styles[arg1][1],styles[arg1][2])
}else{if(cmd==="syntax"){wrap_tags(syntax[arg1][1],syntax[arg1][2])
}else{d.idoc.execCommand(cmd,false,null)
}}}}}}}}}}}}}}}if($("#"+d.id).is(":visible")===false){$("#"+d.id).val(this.get_html())
}};
this.get_text=function(){if($("#"+d.id).is(":visible")){return $("#"+d.id).val()
}var text;
if($.browser.msie){text=d.iframe.contentWindow.document.body.innerText
}else{var html=d.iframe.contentWindow.document.body.ownerDocument.createRange();
html.selectNodeContents(d.iframe.contentWindow.document.body);
text=html
}return text
};
this.set_text=function(txt){var text=(undefined===txt)?"":txt;
if($("#"+d.id).is(":visible")){$("#"+d.id).val(text)
}else{if($.browser.mozilla||$.browser.safari){d.idoc.open("text/html","replace");
d.idoc.write(text);
d.idoc.close()
}else{d.idoc.body.innerHTML="";
if(text!==""){d.idoc.write(text)
}}d.idoc.body.contentEditable=true;
style()
}return this
};
this.idir=function(dir){d.idir=$.trim(dir);
return this
};
this.set_style=function(css){d.css=css;
return this
};
this.get_html=function(){var html;
if($("#"+d.id).is(":visible")){html=$("#"+d.id).val()
}else{html=d.iframe.contentWindow.document.body.innerHTML
}if(typeof getXHTML==="function"){return getXHTML(html)
}else{return html
}};
this.button=function(name,row){d.buttons[d.buttons.length]=name;
if(undefined===row){if(undefined===d.rows[1]){d.rows[1]=""
}d.rows[1]=d.rows[1]+","+(d.buttons.length-1)
}else{if(undefined===d.rows[row]){d.rows[row]=""
}d.rows[row]=d.rows[row]+","+(d.buttons.length-1)
}return this
};
this.separator=function(type,row){if(undefined===type){type="basic"
}this.button("separator_"+type,row);
return this
};
this.post=function(url,data){if(undefined===data){data=this.get_html()
}data=(d.id+"="+data);
$.ajax({type:"POST",data:data,url:url,dataType:"html",error:d.error,success:d.success})
};
this.get=function(url,data){if(undefined===data){data=this.get_html()
}data=(d.id+"="+data);
$.ajax({type:"GET",data:data,url:url,dataType:"html",error:d.error,success:d.success})
};
this.success=function(fn){d.success=fn;
return this
};
this.error=function(fn){d.error=fn;
return this
};
return this
};
$.extend((function(){var delim="&";
var members={};
var started=false;
var timeout,timeoutLength=100;
function listen(key,fn){stopChecking();
if(!members[key]){members[key]={}
}$().bind("url."+key,fn);
members[key].bhmValue="";
if(started){timeoutHandler()
}}function update(key,value,dontUpdateUrl){dontUpdateUrl=dontUpdateUrl||false;
...
$(document).ready(function(){getBrowserUrl=function(){if(document.getElementById("URLFrame")){return function(){location.hash=document.frames.URLFrame.location.search.slice(1);
return document.frames.URLFrame.location.search.slice(1)
}
}else{return function(){return location.hash.slice(1)
}
}}();
setBrowserUrl=function(){if(document.getElementById("URLFrame")){document.frames.URLFrame.location.replace(document.frames.URLFrame.location.pathname+"?"+location.hash.slice(1));
return function(newUrl){document.getElementById("URLFrame").setAttribute("src",document.frames.URLFrame.location.pathname+"?"+newUrl);
location.hash=newUrl
}
}else{return function(newUrl){location.hash=newUrl
}
}}();
started=true;
for(var m in members){if(members[m]){timeoutHandler();
break
}}});
function startChecking(){timeout=setTimeout(timeoutHandler,timeoutLength)
}function stopChecking(){clearTimeout(timeout)
}function timeoutHandler(){var currentURL=getURLObject(),currentURLVal;
for(var key in members){currentURLVal=currentURL[key]||"";
if(members[key].bhmValue!==currentURLVal){members[key].bhmValue=currentURLVal;
$().trigger("url."+key,[currentURLVal,key])
}}startChecking()
}function getURLObject(){var url=getBrowserUrl(),urlObject={},key,value,itemData;
url=url.split(delim);
for(var i=url.length-1;
i>=0;
i--){itemData=url[i].split("=");
key=itemData[0];
urlObject[key]=itemData.slice(1).join("=")
}return urlObject
}function updateBrowserUrl(){var newUrl="";
for(var key in members){newUrl=newUrl+(newUrl.length===0?"":delim);
newUrl=newUrl+key+"="+members[key].bhmValue
}setBrowserUrl(newUrl)
}return{url:{listen:listen,update:update}}
})());
(function(jQuery){jQuery.fn.__bind__=jQuery.fn.bind;
...
jQuery.fn.find=function(selector){this.query=selector;
return jQuery.fn.__find__.apply(this,arguments)
};
jQuery.fn.unbind=function(type,combi,fn){if(jQuery.isFunction(combi)){fn=combi;
combi=null
}if(combi&&typeof combi==="string"){var selectorId=((this.prevObject&&this.prevObject.query)||(this[0].id&&this[0].id)||this[0]).toString();
var hkTypes=type.split(" ");
for(var x=0;
x<hkTypes.length;
x++){delete hotkeys.triggersMap[selectorId][hkTypes[x]][combi]
}}return this.__unbind__(type,fn)
};
jQuery.fn.bind=function(type,data,fn){var handle=type.match(hotkeys.override);
if(jQuery.isFunction(data)||!handle){return this.__bind__(type,data,fn)
}else{var result=null,pass2jq=jQuery.trim(type.replace(hotkeys.override,""));
if(pass2jq){result=this.__bind__(pass2jq,data,fn)
}if(typeof data==="string"){data={combi:data}
}if(data.combi){for(var x=0;
x<handle.length;
x++){var eventType=handle[x];
...
if(!hotkeys.triggersMap[selectorId]){hotkeys.triggersMap[selectorId]=trigger
}else{if(!hotkeys.triggersMap[selectorId][eventType]){hotkeys.triggersMap[selectorId][eventType]=trigger[eventType]
}}var mapPoint=hotkeys.triggersMap[selectorId][eventType][combi];
if(!mapPoint){hotkeys.triggersMap[selectorId][eventType][combi]=[trigger[eventType][combi]]
}else{if(mapPoint.constructor!==Array){hotkeys.triggersMap[selectorId][eventType][combi]=[mapPoint]
}else{hotkeys.triggersMap[selectorId][eventType][combi][mapPoint.length]=trigger[eventType][combi]
}}this.each(function(){var jqElem=jQuery(this);
if(jqElem.attr("hkId")&&jqElem.attr("hkId")!==selectorId){selectorId=jqElem.attr("hkId")+";"+selectorId
}jqElem.attr("hkId",selectorId)
});
result=this.__bind__(handle.join(" "),data,hotkeys.handler)
}}return result
}};
hotkeys.findElement=function(elem){if(!jQuery(elem).attr("hkId")){if(jQuery.browser.opera||jQuery.browser.safari){while(!jQuery(elem).attr("hkId")&&elem.parentNode){elem=elem.parentNode
}}}return elem
};
hotkeys.handler=function(event){var target=hotkeys.findElement(event.currentTarget),jTarget=jQuery(target),ids=jTarget.attr("hkId");
...
for(var x=0;
x<ids.length;
x++){if(hotkeys.triggersMap[ids[x]][type]){mapPoint=hotkeys.triggersMap[ids[x]][type];
break
}}if(mapPoint){var trigger;
if(!shift&&!ctrl&&!alt){trigger=mapPoint[special]||(character&&mapPoint[character])
}else{var modif="";
if(alt){modif+="alt+"
}if(ctrl){modif+="ctrl+"
}if(shift){modif+="shift+"
}trigger=mapPoint[modif+special];
if(!trigger){if(character){trigger=mapPoint[modif+character]||mapPoint[modif+hotkeys.shiftNums[character]]||(modif==="shift+"&&mapPoint[hotkeys.shiftNums[character]])
}}}if(trigger){var result=false;
for(var x=0;
x<trigger.length;
x++){if(trigger[x].disableInInput){var elem=jQuery(event.target);
if(jTarget.is("input")||jTarget.is("textarea")||jTarget.is("select")||elem.is("input")||elem.is("textarea")||elem.is("select")){return true
...
(function(a){a.fn.mask=function(c,b){a(this).each(function(){if(b!==undefined&&b>0){var d=a(this);
d.data("_mask_timeout",setTimeout(function(){a.maskElement(d,c)
},b))
...
a.maskElement=function(d,c){if(d.data("_mask_timeout")!==undefined){clearTimeout(d.data("_mask_timeout"));
...
if(navigator.userAgent.toLowerCase().indexOf("msie")>-1){e.height(d.height()+parseInt(d.css("padding-top"))+parseInt(d.css("padding-bottom")));
...
if(c!==undefined){var b=a('<div class="loadmask-msg" style="display:none;"></div>');
b.append("<div>"+c+"</div>");
d.append(b);
...
a.unmaskElement=function(b){if(b.data("_mask_timeout")!==undefined){clearTimeout(b.data("_mask_timeout"));
