...
// Default Settings	
	jQuery.plugins.settings = {
		...
		target : jQuery('head',this.context),
		init : function(){},
		preLoad : function(){  },
		postLoad : function(){ }
	};
	
	var defaults = jQuery.plugins.settings,
		cache = jQuery.plugins.cache;
...
// Plugin Constructor
	...
	Plugin.prototype.getFile = function(url /* string */){
		if(!url || typeof url != 'string') { throw new Error('jQuery.plugin.getFile(url) - url {String} must be specified') ;}
		var that = this,
		...
		if(extension != 'css' && extension != 'js') {
			throw new Error( 'jQuery.plugin.getFile(url) - Invalid extension:'+ extension + '\n\t'+url); return this;}
	...
	// If no caching or file is not cached
		...
		// Handle Styles	
			if(extension == 'css') {
				(function(){
					var opts = jQuery.extend({url: url},defaults.ajax),
		...
		// Handle Scripts	
				...
				(function(){
					var opts = jQuery.extend({dataType: "script",url: url},defaults.ajax),
						onSuccess = opts.success || function(){};
	...
	Plugin.prototype.afterGet = function(url/* string */){
		var that = this,
			...
			index = jQuery.inArray(url,this.queue);
			
	// Try to catch some weird situtation
	// where postLoad is called for a file that was not in queue
		if(index == -1) {
			throw new Error('jQuery.plugin.afterGet(url) - Ignoring postLoad for file that should not be in queue:\n '+url); return this; }
	...
	// Load each file specified for this plugin
		for(var i=0;i<files.length;i++){
			...
			// For some reason Opera needs a timer here	
				if(jQuery.browser.opera) { setTimeout(function(){ getFile(file); },500); }
				else {getFile(file); } 
			})();
		}
	...
	// and don't go through the rest
		for(var i=0;i<selectors.length;i++){
			...
			if( jQuery(selector,that.context).length > 0 ) { isNeeded = true; break; }
		};
	...
	jQuery.extend(jQuery, {		
		plugin : function(name /* string */,param /* object literal or function */){
			var self = jQuery.plugin;
		// If no arguments passed, get all plugins		
			if (arguments.length == 0) {
				for(var i in jQuery.plugins) { if(i == 'settings' || i == 'cache') { continue;} jQuery.plugins[i].get();};			
				return self; }
				
		// First argument must be a string, the plugin's name
			else if ( typeof name != 'string' ) {
				throw new Error('jQuery.plugin(name,[settings||callback])\n\t\t@param name\t\t{String}\n\t\t@param settings\t{Object}\n\t\t@param callback\t{Function}');
		...
		// Otherwise either return plugin instance or get plugin's files
				...
				var plugin = jQuery.plugins[name];
				if(typeof plugin != 'object') { throw new Error('jQuery.plugin: '+name+' is not specified'); return self; }
