...
import org.castafiore.ui.ex.layout.template.TemplateComponent;
...
public final class CastafioreEngine {
	...
	public synchronized void executeInterceptorOnRender(
			Interceptor interceptor, Container container) {
	...
	public synchronized String getJQuery(Container container, String parentId,
		...
		if (!container.rendered()) {
			...
			Interceptor[] interceptors = interceptorRegistry
					.getInterceptors(container);
			...
			if (!(container.getParent() instanceof TemplateComponent)) {
				...
				creationCommand = Constant.NO_CONFLICT + "(\""
						+ Constant.ID_PREF + root.getId() + " *[__path^='"
						+ container.getAttribute("__path") + "']\").remove();";
			}
			...
			creationCommand = creationCommand
					+ ClientProxy.getCreationCommand(container, parentId, html,
		...
		if (false) {
			...
			//
			if (container.rendered()) {
				jQuery.appendJSFragment("var _" + container.getId()
						+ "= document.getElementById('" + container.getId()
						+ "');");
			}
		...
		if (container.hasChildren()) {
			Iterator<Container> iterChildren = container.getChildren()
					.iterator();
		...
		if (!container.rendered()) {
			ClientProxy ready = new ClientProxy(container,
		...
		String script = creationCommand + builder.toString();
	 ...
	 */
	public synchronized String executeServerAction(String componentId,
		...
		if (component != null) {
			...
			// logger.warn("the component " + componentId +
			// " cannot be found in dom. Possibly deleted");
		...
		if (event != null && event.hashCode() == eventId) {
			try {
				...
				try {
					...
					EXWarning warning = root
							.getDescendentOfType(EXWarning.class);
					if (warning == null) {
				...
				if (updateClient) {
					String jQuery = getJQuery(application, parentId,
							application,
							new ListOrderedMap<String, List<KeyValuePair>>());
					result.append(jQuery);
				}

				ClientProxy wrapper = new ClientProxy(component,
						new ListOrderedMap<String, List<KeyValuePair>>());
	...
	private synchronized Container[] getPrevAndNextSibblings(Container container) {
		...
		if (container.getParent() != null) {
			...
			for (i = 0; i < sibblings.size(); i++) {
				if (container.getParent().getChildren().get(i).getId()
	 ...
	 * @param jQuery
	 */
	public synchronized void executeClientPartOfEvents(Container container,
			Container root, ClientProxy jQuery) {
		...
		while (eventTypesIterator.hasNext()) {
			...
			ClientProxy rootWrapper = new ClientProxy(root,
					new ListOrderedMap<String, List<KeyValuePair>>());

			ClientProxy wrapper = rootWrapper.getDescendentById(container
					.getId());
