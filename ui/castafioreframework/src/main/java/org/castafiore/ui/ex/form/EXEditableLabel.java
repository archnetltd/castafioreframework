 ...
 * Oct 22, 2008
 */
public class EXEditableLabel extends AbstractStatefullComponent implements Event {
	...
	public void ClientAction(ClientProxy application) {
		...
		else
		{
			...
			if(getAncestorOfType(EXEditableLabel.class).getEvents().containsKey(Event.CHANGE) && getAncestorOfType(EXEditableLabel.class).getEvents().get(Event.CHANGE).size() > 0)
	...
	public boolean ServerAction(Container component,
		...
		return component.getAncestorOfType(EXEditableLabel.class).getEvents().get(Event.CHANGE).get(0).ServerAction(component.getAncestorOfType(EXEditableLabel.class), requestParameters);
