...
public class ImageUtil {
	...
	public static BufferedImage applyFilter(BufferedImage img,BufferedImageOp filter){
	...
	private static void testLayout()throws Exception{
		...
		for(File f : imgs){
			...
			for(int i = 0; i <  width;i++){
				for(int j =0; j < height; j++){
					...
					if(red > 200 || green > 200 || blue > 200 || red < 25 || green < 25 || blue < 25){
						...
						Color c1 = new Color((red*60)%255,(red*60)%255 , (blue*45)%255);
						//Color c1 =c.brighter();
			...
			//GradientFilter filter = new GradientFilter(new Point(0, 0),new Point(0,height),new Color(r1, g1, b1).getRGB(),new Color(r2,g2, b2).getRGB(),false,GradientFilter.LINEAR,GradientFilter.LINEAR);
...
public static void resizeImage(InputStream image, int width, String extension, OutputStream out) throws IOException{
	...
	private static BufferedImage resizeImage(BufferedImage originalImage, int width){
		...
		BufferedImage resizedImage = new BufferedImage(width, nh.intValue(), type);
...
//		String templateDir = "C:\\Users\\kureem\\castafiorea\\casta-mysite\\web\\youdo\\menu\\template";
//		Random rand = new Random();
//		String template = IOUtil.getFileContenntAsString(templateDir + "\\menu7.html");
//		for(int i = 0; i < 100; i ++){
...
//			String resultName = "C:\\Users\\kureem\\castafiorea\\casta-mysite\\web\\youdo\\menu\\" + r1 + g1 + b1;
...
//				if(j == 0)
//					saveImage(applyFilter(templateDir + "\\" + imgs[j], new GradientFilter(new Point(0, 0),new Point(174,42),new Color(r1, g1, b1).getRGB(),new Color(r2,g2, b2).getRGB(),false,GradientFilter.BILINEAR,GradientFilter.INT_LINEAR)), resultName + "\\"+ imgs[j]);
//				else{
//					BufferedImage img = loadImage(templateDir + "\\" + imgs[0]);
//					BufferedImage re = new BufferedImage(img.getWidth(), img.getHeight(), BufferedImage.TYPE_INT_RGB);
//					
//					BufferedImageOp op =new GradientFilter(new Point(0, 0),new Point(174,42),new Color(r1, g1, b1).getRGB(),new Color(r2,g2, b2).getRGB(),false,GradientFilter.BILINEAR,GradientFilter.INT_LINEAR);
//					op.filter(img, re);
...
//		}
//		
//		FileOutputStream out = new FileOutputStream("C:\\Users\\kureem\\castafiorea\\casta-mysite\\web\\youdo\\menu" + "\\menu.html");
//		template = "<html><head></head><body><div>" + complete.toString() + "</div></body></html>";
