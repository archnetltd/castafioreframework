...
public class ComponentUtil {
	 ...
	 * 
	 * @param container
	 *            - The root container to start digging
	 * @param result
	 *            - Will contain all {@link FormComponent}
	 * @param ccs
	 *            - will contain the component with id = componentId
	 * @param componentId
	...
	/**
	 * Get all {@link FormComponent}s into the list of result
	 * 
	 * @param container
	 *            - The root component to start digging
	 * @param result
	...
	/**
	 * Iterates over all components including the specified one that are
	 * assignable from the specified type and executes the visitor
	 * 
	 * @param container
	 *            - The root container to start digging
	 * @param type
	 ...
	 * @param visitor
	 *            - The visitor to execute
	 */

	public static void iterateOverDescendentsOfType(Container container,
			Class<?> type, ComponentVisitor visitor) {
	...
	/**
	 * Search all descendents of the specified type of the specified container
	 * into the list of result
	 * 
	 * @param container
	 *            - the root container to start searching
	 * @param result
	 *            - The list to put all the containers meeting the specified
	 *            type
	 * @param type
	 *            - The type to search for
	 */
	public static void getDescendentsOfType(Container container,
			List<Container> result, Class<?> type) {
		if (type.isAssignableFrom(container.getClass())) {
			result.add(container);
		}
	...
	/**
	 * Searches a container starting from the specified root having the
	 * specified componentId
	 * 
	 * @param root
	 *            - the root container to start searching
	 * @param componentId
	 *            - the component id to search
	 ...
	 * 
	 * @param c
	 *            - the container to apply the styles
	 * @param styles
	 *            - the styles to apply
	 */
	...
	/**
	 * Build a new primitive container with the specified name, specified tag,
	 * text and style class
	 * 
	 * @param name
	 *            - Name of component. Cannot be null
	 * @param tagName
	 *            - Tag name of component. Cannot be null
	 * @param text
	 *            - The text of the component. Can be null
	 * @param sclass
	 *            - The style class to apply on the component. Can be null
	 * @return
	 */
	public static Container getContainer(String name, String tagName,
			String text, String sclass) {
	...
	/**
	 * Utility method helping to apply a style on all descendents of a container
	 * of the specified type
	 * 
	 * @param ancestor
	 *            - The root container
	 * @param type
	 *            - The descendents od type
	 * @param styleName
	 *            - The style name
	 * @param styleValue
	 *            - The value of the style for the style name
	 */
	public static void applyStyleOnAll(Container ancestor,
			Class<? extends Container> type, String styleName, String styleValue) {
	 ...
	 * 
	 * @param request
	 *            - The request of the page into which to render the application
	 * @param response
	 *            - The response of the page into which to render the
	 *            application
	 * @throws ServletException
	 *             - Whenever an error occures
	 */
	public static void loadApplication(HttpServletRequest request,
		...
		if (applicationId != null) {
			...
			Application application = (Application) request.getSession()
			...
			if (application == null && applicationId != null) {
				...
				// " not found in session. Loading fresh application from registry");
				application = context.getBean(ApplicationRegistry.class)
						.getApplication(request, response);
				if (application == null) {
					// logger.error("there is no bean with name " +
					// applicationId + " configured in application registry");
					throw new ServletException("there is no bean with name "
							+ applicationId
							+ " configured in application registry");
				}
			...
			application.getConfigContext().put("contextPath",
					((HttpServletRequest) request).getContextPath());
			application.getConfigContext().put("serverName",
					request.getServerName());
			application.getConfigContext().put("serverPort",
					request.getServerPort());
			application.getConfigContext().put("session", request.getSession());
			application.getConfigContext().put("remoteAddress",
					request.getRemoteAddr());
			application.getConfigContext().put("sessionid",
			...
			CastafioreApplicationContextHolder
			...
			if (application instanceof WebServletAwareApplication) {
				...
				((WebServletAwareApplication) application)
	...
	public static void moveDown(Container component, Container layout) {
		...
		if (index == layout.getChildren().size() - 1) {
			throw new UIException(
					"Cannot move downward. Already at end of list");
		}
		layout.getChildren().remove(index);
		layout.getChildren().add(index + 1, component);
		layout.setRendered(false);
	...
	// panel.removeClass("ui-widget-content").addClass("ui-widget-header").setStyle("padding",
	// "0").setStyle("background-position", "0 0");
	// String title = panel.getDescendentByName("title").getText();
	// panel.getDescendentByName("titleBar").getChildByIndex(0).remove();
	// panel.getDescendentByName("titleBar").setText(title).setStyleClass("").setStyle("text-align",
	// "center").setStyle("padding", "10px");
	// panel.getDescendentByName("content").setStyleClass("ui-widget-content").setStyle("margin",
	// "10px").setStyle("padding", "0").setStyle("overflow",
	// "auto").setStyle("font-weight", "normal");
	// panel.getDescendentByName("panelFooter").setStyle("margin", "10px");
	// panel.getDescendentByName("closeButton").setStyleClass("ui-state-default").setStyle("position",
	// "absolute").setStyle("top", "3px").setStyle("right", "3px")
	...
	// explorer.getDescendentByName("addressToolBar").setStyle("border",
	// "none").removeClass("ui-corner-all").removeClass("ui-corner-top");
	// if(explorer.getDescendentByName("ThumbnailView") != null)
	// explorer.getDescendentByName("ThumbnailView").removeClass("ui-corner-all").setStyle("margin",
	// "10px");
	// }
	//
	...
	// des.getDescendentByName("digner").removeClass("ui-corner-all").setStyle("border-bottom",
	// "none");
	// des.getDescendentByName("desToolbar").removeClass("ui-corner-all").setStyle("border-bottom",
	// "none");
