...
/**
 * The event model in castafiore It is split into 3 parts due to the fact that
 * in a web application codes can be executed on the browser, and on the server
 * as well
 * 
 * Of course the Success method is executed after a ServerAction
 * 
 * request data requestData ClientAction ----------------->
...
public interface Event extends Serializable {
	...
	/**
	 * This method represents the client part of an event. This method is
	 * executed only once on rendering of the component. {@link ClientProxy}
	 * provides a nice abstraction for generating javascript codes. It is the
	 * browser version of the component
	 * 
	 * It is this method that is responsible in starting a server request to be
	 * executed in the serverAction method
	 * 
	 * This allows user to make actions that does not require server actions
	 * 
	 * @param container
	 *            - A proxy for the browser
	 */
	 ...
	 * 
	 * The browser will be update only if this method returns true This allows
	 * users to make server action, that does not require update of the client
	 * 
	 * @param container
	 *            - the container on which the event was registered
	 * @param request
	 *            - request parameters that can be passed in the
	 *            {@link ClientProxy.makeServerRequest}
	...
	/**
	 * This method is executed after each server action Basically it is the
	 * javascript that may be required after a server action
	 * 
	 * @param container
	 *            - again, this is the browser version of the component on which
	 *            the event was registered
	 * @param request
	 *            - request parameters that can be added in the serverAction
	 *            method
	 * @throws UIException
	 */
