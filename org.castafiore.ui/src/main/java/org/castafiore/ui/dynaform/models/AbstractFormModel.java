...
package org.castafiore.ui.dynaform.models;
...
import org.castafiore.ui.dynaform.Field;
import org.castafiore.ui.dynaform.FormModel;
import org.castafiore.ui.dynaform.ValidatorConfig;
...
public abstract class AbstractFormModel<T> implements FormModel {
	...
	public String validate(FormComponent component, Field field)
			throws Exception {
	...
	public abstract Button getActionAt(int index, DynaForm form);

	@SuppressWarnings({ "rawtypes", "serial" })
	private FormComponent buildComponent(Field fieldAnnotation)
			throws Exception {
		...
		if (listproviderClass.equals(void.class)) {
			...
			ListProvider lProvider = (ListProvider) listproviderClass
					.newInstance();
		...
		FormComponent component = (FormComponent) o;
	...
	public FormComponent getFieldAt(int index, DynaForm form) {
		...
		try {
			...
			FormComponent component = buildComponent(fieldAnnotation);
			...
			if (bean_ != null) {
				if (m.getName().startsWith("get")) {
					value = m.invoke(bean_, (Object[]) null);
				} else if (m.getName().startsWith("is")) {
					value = m.invoke(bean_, (Object[]) null);
				} else if (m.getName().startsWith("set")) {
					if (m.getReturnType().equals(Boolean.class)) {
						value = classType_
								.getMethod("is" + m.getName().substring(3),
										(Class[]) null).invoke(bean_,
										(Object[]) null);
						...
						value = classType_.getMethod(
								"get" + m.getName().substring(3),
								(Class[]) null).invoke(bean_, (Object[]) null);
					}
					// setter
				}
			}
			...
			throw ExceptionUtil.getRuntimeException(
					"cannot instantiate the fieldType of type:" + inputType, e);
		}
	}
