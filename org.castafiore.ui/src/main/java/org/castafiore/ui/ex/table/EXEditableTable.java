...
import org.castafiore.ui.table.EditableCellRenderer;
import org.castafiore.ui.table.EditableTable;
import org.castafiore.ui.table.EditableTableModel;
 ...
 * 
 */
public class EXEditableTable extends EXTable implements Event, EditableTable {
	 ...
	 * @param model
	 *            The {@link EditableTableModel} to apply on the table
	 * @param renderer
	 *            The {@link EditableCellRenderer} to apply on the table
	 */
	public EXEditableTable(String name, EditableTableModel model,
			EditableCellRenderer renderer) {
		...
		for (int row = 0; row < rowsToShow; row++) {
			...
			for (int col = 0; col < tableModel_.getColumnCount(); col++) {
				...
				if (tr.getChildren().size() <= col) {
					td = new EXContainer("", "td").setAttribute("c", col + "")
							.setAttribute("r", row + "").setAttribute("m", "r");
	...
	public void startEdit(int row, int col, int page) {
		...
		Container container = getChild("tbody").getChildByIndex(row)
				.getChildByIndex(col);

		container.getChildren().clear();
		container.addChild(sft.addEvent(this, BLUR).setAttribute("r", row + "")
				.setAttribute("c", col + "").setAttribute("m", "e"));
	...
	public void cancelEdit(FormComponent stf) {
		...
		Container c = celleRendere_.getComponentAt(row, col, page, tableModel_,
				this);
	...
	public boolean ServerAction(Container container, Map<String, String> request)
		...
		if (!editMode) {
			...
			container.getChildren().clear();
			container.addChild(sft.addEvent(this, BLUR)
					.setAttribute("r", row + "").setAttribute("c", col + "")
					.setAttribute("m", "e"));
			container.getEvents().clear();
		} else {
			Container td = container.getParent();
			Object value = container.getAncestorOfType(FormComponent.class)
					.getValue();
			((EditableTableModel) tableModel_)
					.setValueAt(col, row, page, value);
			Container c = celleRendere_.getComponentAt(row, col, page,
					tableModel_, this);
			td.addEvent(this, CLICK);
			td.addChild(c);
