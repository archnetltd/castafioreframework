...
public class EXList<T> extends AbstractEXList<T> implements ListItemRenderer<T> {
	...
	public void addItem(final ListItem<T> item) {
		addChild(new EXContainer(getChildren().size() + "", "li").setStyle(
				"list-style", "none").addChild(item));
		item.addEvent(new Event() {
			...
			@Override
			public boolean ServerAction(final Container container,
					Map<String, String> request) throws UIException {
				final EXList<?> exList = container
						.getAncestorOfType(EXList.class);
				exList.setAttribute("value", container.getParent().getName());
				ComponentUtil.iterateOverDescendentsOfType(exList,
						EXListItem.class, new ComponentVisitor() {

							@SuppressWarnings({ "rawtypes", "unchecked" })
							@Override
							public void doVisit(Container c) {
								exList.selectItem((EXListItem) c, c.getName()
										.equalsIgnoreCase(container.getName()));
