...
public class EXBorderLayout extends EXXHTMLFragment implements LayoutContainer,
		PopupContainer {
	...
	public final static String[] CONTENT_NAMES = {
			"EXBorderLayoutContainertop", "EXBorderLayoutContainerleft",
			"EXBorderLayoutContainercenter", "EXBorderLayoutContainerright",
	...
	protected EXBorderLayout(String name, String template) {
		super(name, template);
		Container c = ComponentUtil.getContainer("popupContainer", "div", null,
				null);
		c.setStyle("position", "absolute");
	...
	public void validateLayoutData(String layoutData)
		...
		try {
			if (layoutData != null
					&& Integer.parseInt(layoutData.toString()) >= 0
					&& Integer.parseInt(layoutData.toString()) < CONTENT_NAMES.length) {
				return;
			} else {
				throw new InvalidLayoutDataException("the layout data "
						+ layoutData + " passed in the layoutContainer "
						+ getClass() + "is invalid");
			}
		} catch (Exception e) {
			throw new InvalidLayoutDataException("the layout data "
	...
	public List<DroppableSection> getSections() {
		...
		for (int i = 0; i < CONTENT_NAMES.length; i++) {
			...
			if (c.getChildren().size() > 0) {
				DroppableSection section = new DroppableSection(c
						.getChildByIndex(0).getId(), c.getChildByIndex(0)
						.getName(), i);
