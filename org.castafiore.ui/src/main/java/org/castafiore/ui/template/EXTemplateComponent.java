...
public abstract class EXTemplateComponent extends EXContainer implements
	...
	public String event(int type, Map<String, String> params) {
		...
		if (evts.size() == 0) {
			throw new UIException("There is no event of type "
					+ EventUtil.getEventName(type)
	...
	public String getHTML() {
		...
		try {
			String html = this.getCompiler().compile(getTemplate(),
					getContext());
			// if(forceStyle){
			...
			return "<h1>There was an error<h1><p>"
					+ JavascriptUtil.javaScriptEscape(e.getMessage()) + "</p>";
	...
	public String getTemplate() {
		try {
			if (getRoot().getServerName().equals("localhost")) {
				return ResourceUtil.getTemplate(getTemplateLocation(),
						getRoot());
			}
			...
			if (bean != null) {
				if (bean.containsKey(templateLocation)) {
					...
					String template = ResourceUtil.getTemplate(
							templateLocation, getRoot());
