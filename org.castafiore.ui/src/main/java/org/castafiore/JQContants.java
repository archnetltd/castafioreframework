...
public interface JQContants {
		...
		public boolean ServerAction(Container container,
			...
			for (Container tab : panelTabs.getChildren()) {
				...
				if (index.equals(oTab)) {
					...
					panel.getTabRenderer().onSelect(panel, model,
							Integer.parseInt(oTab), tab);
				} else {
					tab.setAttribute("class", TABS_INACTIVE_TAB_STYLE);
					content.setDisplay(false);
					panel.getTabRenderer().onDeselect(panel, model,
							Integer.parseInt(oTab), tab);
				}
			}
			return true;
	...
	public final static Event CLOSE_EVENT = new Event() {
		...
		public boolean ServerAction(Container container,
				Map<String, String> request) throws UIException {
			container.getAncestorOfType(org.castafiore.ui.panel.Panel.class)
					.remove();
			return true;
		}
