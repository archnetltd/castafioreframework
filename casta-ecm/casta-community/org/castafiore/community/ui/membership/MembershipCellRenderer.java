...
public class MembershipCellRenderer extends DefaultCellRenderer {
	...
	public Container getComponentAt(int row, int column,int page, TableModel model,
		...
		if(column <= 1){
			...
			if(column == 2){
				...
				c.addEvent(CommunityEvents.SHOW_MEMBERSHIP_FORM_EVENT, Event.CLICK);
			}else{
				span.addClass("ui-icon-circle-close");
				c.addEvent(CommunityEvents.DELETE_MEMBERSHIP_EVENT, Event.CLICK);
			}
			c.addChild(span);
			c.setAttribute("rolename", value.toString());
			return c;
		}
	...
	public void onChangePage(Container component,  int row,	int column,int page, TableModel model, EXTable table) 
	{
		...
		if(column <= 1){
			...
			span.setAttribute("rolename", newValue.toString());
		}
